...
class ArduinoDataRead {
    ...
    fake_data(){
        setInterval(() => {
            let data_float = sensors.dht11({minHum:30, maxHum:90});
            ...
            console.log('Luminosidade: ', parseFloat(data_float[1].toFixed(2)), 'Humidade: ', data_float[0]);
    ...
    SetConnection() {
        SerialPort.list().then(listSerialDevices => {
            ...
            parser.on('data', (data) => {
				...
				let luminosidade = parseFloat(value[1].replace('\r', ''));
				let humidity = parseFloat(value[0].replace('\r', ''));
                this.listData.push(humidity);
                this.__listDataTemp.push(luminosidade)
                console.log("Luminosidade: ",luminosidade," Umidade: ",humidity);
            });
